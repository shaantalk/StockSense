...
export const PreferredCurrencyManager = ({ config }: Pick<SharedSettingsProps, 'config'>) => {
    const currentUserEmail = config?.currentUser?.email || '';
    const member = config?.members?.find(m => m.email === currentUserEmail);
    const initialCurrency = member?.preferredCurrency || config?.currency || '';

    const [showModal, setShowModal] = useState(false);
    const [currencyInput, setCurrencyInput] = useState(initialCurrency);
    ...
    const handleUpdate = async () => {
        if (!currencyInput.trim() || !currentUserEmail) return;
        setLoading(true);
        try {
            await googleApiService.updateMemberProfile(currentUserEmail, { preferredCurrency: currencyInput.trim() });
            setShowModal(false);
    ...
    return (
        <>
            <SettingItem
                ...
                value={(() => {
                    const lookupCurrency = member?.preferredCurrency || config?.currency || '';
                    if (!lookupCurrency) return 'Not Set';
                    const selected = currencies.find(c => `${c.code}_${c.countryName}` === lookupCurrency) || currencies.find(c => c.code === lookupCurrency);
                    if (selected) {
                        return `${selected.countryName} (${selected.symbol}) ${selected.code}`;
                    }
                    return `${getCurrencySymbol(lookupCurrency)} - ${getCurrencyName(lookupCurrency)}`;
                })()}
                    ...
                    >
                        ...
                        >
                            <div className="text-center space-y-2">
                                ...
                                <h3 className="text-xl font-black text-white tracking-tight">Preferred Currency</h3>
                                <p className="text-xs text-slate-400 font-medium">Select your personal shopping currency.</p>
                            </div>
