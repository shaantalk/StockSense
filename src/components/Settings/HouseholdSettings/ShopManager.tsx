...
import { Store, Plus, X, Loader2, CheckCircle2 } from 'lucide-react';
import { googleApiService } from '../../../services/googleApiService';
import type { SharedSettingsProps } from '../types';

export const ShopManager = ({ config, setDeleteConfig }: Pick<SharedSettingsProps, 'config' | 'setDeleteConfig'>) => {
    ...
    const handleAdd = async () => {
        ...
        setLoading(true);
        try {
            await googleApiService.addShop(newName.trim(), newColor);
            setNewName('');
